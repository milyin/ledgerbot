...
pub trait PeriodsStorageTrait: Send + Sync {
    ...
    /// Load period by name
    async fn load_current_period(&self, chat_id: ChatId) -> Option<Vec<String>> {
        self.load_period(chat_id, CURRENT_PERIOD_NAME).await
    }

    /// Save period by name
    async fn save_current_period(&self, chat_id: ChatId, data: Vec<String>) {
        self.save_period(chat_id, CURRENT_PERIOD_NAME, data).await;
    }
}
