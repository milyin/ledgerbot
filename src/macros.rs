            ...
            match current_char {
                ...
                b'{' => {
                    ...
                    if !in_code && !in_pre && !is_escaped && !is_format_placeholder {
                        panic!("Unescaped '{{' in MarkdownV2 format string. Use \\{{ to escape it or use {{}} for format placeholders.");
                    }
                },
                b'}' => {
...
macro_rules! send_message_markdown {
    ($bot:expr, $chat_id:expr, $format_str:expr $(, $arg:expr)*) => {{
        ...
        // Note: Arguments are automatically escaped by teloxide::utils::markdown::escape()
        // Format string validation is done at compile-time by validate_markdownv2_format()
...
mod tests {
    ...
    fn test_escape_detection() {
        ...
        for (i, &current_char) in bytes.iter().enumerate() {
            ...
            if current_char == b'.' {
                assert!(is_escaped, "Period at position {} should be escaped in '{}'", i, test_string);
            }
